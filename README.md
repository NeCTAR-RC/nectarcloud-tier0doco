...
Copying volumes of content from the internet is to be avoided, if said content
 is authoritative, simply link to it instead.  For example [installing git](https://git-scm.com/book/en/v2/Getting-Started-Installing-Git) 
...
- then you need to commit your changes, adding a useful message which explains 
exactly what your commit is about.  You should consider formatting your commit
message with a less than 10 word subject up the top, then if any further 
explination is required, do that in a separate paragraph underneath.
...
When Jenkins finally accepts your change (see ammending changes below), Jenkins
will automatically vote (or +1) your change.  
