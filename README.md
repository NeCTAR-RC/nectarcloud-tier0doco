...
- using a key generator (`ssh-keygen`), as you will need a keypair for `git review`.
...
- If you don't already have a suitable keypair, then create a new ssh keypair using
  `ssh-keygen -t rsa -f my_gerrit.key` then copy both files to your
  `$HOME/.ssh` directory. You need this for passwordless use of the `git review` command.
  (for the instructions in this document we'll now assume that you have a keypair in your
  `$HOME/.ssh` directory consisting of private key `my_gerrit_key` and corresponding public key
  `my_gerrit_key.pub`)
...
- Create a Gerrit "username" and add the new set of ssh keys. This is done within your
...
#### Cloning the project from ithub <a name='Cloning'></a>
